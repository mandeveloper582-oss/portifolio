    ...
    <section id="faq" className="relative overflow-hidden px-6 py-24 sm:px-8 lg:px-10">
      ...
      <div className="mx-auto max-w-5xl relative">
        <div className="mb-16 text-center">
          ...
          <h2 className="mt-4 text-4xl font-semibold text-(--text-primary) sm:text-5xl">Frequently asked questions about working together.</h2>
          <p className="mx-auto mt-5 max-w-3xl text-base leading-7 text-(--text-secondary)">Answers to the most common questions clients ask before starting a website or web application project.</p>
        </div>
        ...
        <div className="space-y-5">
          {faqs.map((faq) => (
            <motion.div
              ...
              className="rounded-4xl border border-(--border) bg-(--panel) shadow-[0_30px_90px_rgba(14,165,233,0.08)] backdrop-blur-xl"
            >
              <button
              ...
              {active === faq.id && (
                <div className="border-t border-(--border) px-6 pb-6 pt-3">
                  <p className="text-(--text-secondary) leading-8">{faq.answer}</p>
                </div>
              )}
            </motion.div>
          ))}
